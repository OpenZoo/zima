...
public class ZimaLicenseWindow {
    ...
    static String readAllText(String classFile) throws IOException {
        try (InputStream stream = ZimaLicenseWindow.class.getClassLoader().getResourceAsStream(classFile)) {
            ...
            return new String(FileUtils.readAll(stream), StandardCharsets.UTF_8);
        }
    }
    ...
    public ZimaLicenseWindow(JFrame parent, String zimaVersion) throws IOException {
...

        {
            ...
            shortText.append("\n\n");
            shortText.append(readAllText(LICENSE_TEXTS.get(0).getSecond()));
            shortText.append("\n");
            ...
            for (int i = 1; i < LICENSE_TEXTS.size(); i++) {
                ...
                shortText.append(readAllText(LICENSE_TEXTS.get(i).getSecond()));
            }
            addLicenseTab(tabbedPane, "About", shortText.toString());
        }

        for (int i = 0; i < LICENSE_TEXTS.size(); i++) {
            String longText = readAllText(LICENSE_TEXTS.get(i).getThird());
